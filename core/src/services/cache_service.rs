...
impl CacheService {
    ...
    pub fn get<T: FromRedisValue>(&mut self, key: &str) -> AppResult<T> {
        self.redis.get::<T>(key.to_string()).into_app_result()
    }
        ...
        Fun: FnOnce(&mut Self) -> AppResult<Val>,
    {
        ...
        match result {
            Ok(option) => match option {
                ...
                Some(data) => Ok(serde_json::from_str::<Val>(data.to_owned().leak()).unwrap()),
            },
            Err(err) => Err(err),
        }
    }
