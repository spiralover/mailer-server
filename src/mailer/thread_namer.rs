...
fn get_name_lock_file() -> String {
    let dir_binding = env::current_dir().unwrap();
    let dir = dir_binding.to_str().unwrap();
    dir.to_owned() + "/.name_lock"
}

fn last_locked_name() -> Vec<String> {
    let file = File::open(get_name_lock_file()).unwrap();
    let lines: Vec<String> = RevLines::new(BufReader::new(file)).map(|l| l.unwrap()).collect();
    lines
}
